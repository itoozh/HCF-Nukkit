...
public class PvPTimer extends PlayerTimer implements Listener {
    public PvPTimer(TimerManager timerManager) {
        super(timerManager, true, "PvPTimer", "PLAYER_TIMERS.PVP_TIMER", Main.getInstance().getConfig().getInt("TIMERS_COOLDOWN.PVP_TIMER"));
    }
    ...
    public void onTeleport(PlayerTeleportEvent event) {
        ...
        if (this.hasTimer(player)) {
            if (playerTeam != null && team != null && playerTeam.getClaim().getName().equals(team.getName()) && Main.getInstance().getConfig().getBoolean("PVP_TIMER.ENTER_OWN_CLAIM")) {
                return;
            }
            if (team != null) {
                if (team.getType() == ClaimType.TEAM || team.getType() == ClaimType.KOTH) {
                    ...
                    player.sendMessage(TextFormat.colorize(LanguageUtils.getString("PVP_TIMER.CANNOT_TELEPORT").replaceAll("%claim%", team.getNameFormat(player))));
                }
            }
        }
    }
