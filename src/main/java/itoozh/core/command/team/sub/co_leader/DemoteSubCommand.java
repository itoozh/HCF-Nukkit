...
public class DemoteSubCommand extends BaseSubCommand {

    private final List<Role> roles;

    public DemoteSubCommand(String name) {
        super(name);
    ...
    public boolean execute(CommandSender sender, String label, String[] args) {
        ...
        if (!team.getMembers().contains(targetSession.getMember())) {
            sender.sendMessage(TextFormat.colorize(TextFormat.colorize(LanguageUtils.getString("TEAM_COMMAND.TEAM_DEMOTE.NOT_IN_TEAM").replaceAll("%player%", targetPlayer.getName()))));
            return false;
        }
        if (player.getUniqueId().equals(targetPlayer.getUniqueId())) {
            sender.sendMessage(TextFormat.colorize(TextFormat.colorize(LanguageUtils.getString("TEAM_COMMAND.TEAM_DEMOTE.DEMOTE_SELF"))));
            return false;
        }
        ...
        if (member.getRole() == role) {
            sender.sendMessage(TextFormat.colorize(TextFormat.colorize(LanguageUtils.getString("TEAM_COMMAND.TEAM_DEMOTE.LOWEST_ROLE"))));
            return false;
        }
        if (!team.checkRole(player, member.getRole())) {
            sender.sendMessage(TextFormat.colorize(TextFormat.colorize(LanguageUtils.getString("TEAM_COMMAND.TEAM_DEMOTE.HIGHER_ROLE"))));
            return false;
        }
        member.setRole(role);
        sender.sendMessage(TextFormat.colorize(TextFormat.colorize(LanguageUtils.getString("TEAM_COMMAND.TEAM_DEMOTE.DEMOTED_BROADCAST").replaceAll("%player%", targetPlayer.getName()).replaceAll("%role%", member.getRole().getName()))));
