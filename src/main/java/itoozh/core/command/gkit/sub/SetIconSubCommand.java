...
public class SetIconSubCommand extends BaseSubCommand {

    public SetIconSubCommand(String name) {
        super(name);
    }
    ...
    public boolean execute(CommandSender sender, String label, String[] args) {
        if (args.length < 2) {
            sender.sendMessage(TextFormat.RED + "Usage: /gkit seticon [name]");
            return true;
        ...
        gKit.setIcon(player.getInventory().getItemInHand());
        player.sendMessage(TextFormat.GREEN + "You have set the icon for the gkit called " + gKit.getName() + ".");
        return true;
    }
