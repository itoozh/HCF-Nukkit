...
public class SetNameSubCommand extends BaseSubCommand {
    public SetNameSubCommand(String name) {
        super(name);
    }
    ...
    public boolean execute(CommandSender sender, String label, String[] args) {
        ...
        Main.getInstance().getCrateManager().getCrates().remove(crate.getName());
        Main.getInstance().getCrateManager().getCrates().put(newName, crate);
        crate.setName(newName);
        Main.getInstance().getHologramManager().restartHologram();
        sender.sendMessage(TextFormat.colorize("&aName in " + crate.getName() + "crate has been updated!"));
        return true;
    }
