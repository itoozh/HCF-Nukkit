...
public class KeyAllSubCommand extends BaseSubCommand {
    public KeyAllSubCommand(String name) {
        super(name);
    }
    ...
    public boolean execute(CommandSender sender, String label, String[] args) {
        if (args.length < 3) {
            sender.sendMessage(TextFormat.colorize("&cUsage: /place keyall <name> <amount>"));
            return false;
        }
        ...
        sender.sendMessage(TextFormat.colorize("&aSuccessfully has been given x" + amount + " keys to all players."));
        return true;
    }
