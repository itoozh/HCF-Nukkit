...
public class DeleteSubCommand extends BaseSubCommand {
    ...
    public boolean execute(CommandSender sender, String label, String[] args) {
        if (args.length < 2) {
            sender.sendMessage(TextFormat.colorize("&cUsage: /place delete <name>"));
            return false;
        }
        ...
        Main.getInstance().getCrateManager().removeCrate(crate);
        sender.sendMessage(TextFormat.colorize("&aSuccessfully delete " + name + " crate!"));
        return true;
    }
